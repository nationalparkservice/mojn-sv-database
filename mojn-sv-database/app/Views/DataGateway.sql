...
SELECT        S.ID AS SiteID, V.ID AS VisitID, S.Code AS SiteCode, S.Name AS SiteName, P.Code AS ParkCode, 'NA' AS Subunit, V.StartDate, Year(V.StartDate) AS VisitGroup, 
                         lookup.VisitType.Code AS SiteStatus, lookup.SpringCategory.Code AS SampleFrame, DPL.Label AS DPL, lookup.ProtectedStatus_Shared.Label AS SiteProtectedStatus, S.ParkID, V.DataProcessingLevelID, 
...
FROM            data.Site AS S LEFT OUTER JOIN
                         ...
                         lookup.ProtectedStatus_Shared ON S.ProtectedStatusID = lookup.ProtectedStatus_Shared.ID LEFT OUTER JOIN
