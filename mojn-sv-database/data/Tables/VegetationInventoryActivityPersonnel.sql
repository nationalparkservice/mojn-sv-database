CREATE TABLE [data].[VegetationInventoryActivityPersonnel] (
    ...
    CONSTRAINT [FK_VegetationInventoryActivityPersonnel_Personnel] FOREIGN KEY ([PersonnelID]) REFERENCES [ref].[Personnel] ([ID]),
...
GO
EXECUTE sp_addextendedproperty @name = N'MS_Description', @value = 'Foreign key to ref.Personnel (lookup of MOJN personnel)', @level0type = N'SCHEMA', @level0name = N'data', @level1type = N'TABLE', @level1name = N'VegetationInventoryActivityPersonnel', @level2type = N'COLUMN', @level2name = N'PersonnelID';
